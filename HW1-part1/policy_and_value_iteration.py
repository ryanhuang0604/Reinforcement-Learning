...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
	...
	for i in range(max_iterations): 
		...
		for s in range(num_spaces):
			R_sa = [sum([prob * r for prob, s_, r, done in env.P[s][a]]) for a in range(num_actions)]
			Q_sa = [R_sa[a] + gamma * sum([prob * V[s_] for prob, s_, r, done in env.P[s][a]]) for a in range(num_actions)]
			V[s] = np.max(Q_sa)
		if (np.sum(np.fabs(prev_V - V)) <= eps):
			break
			
	# find policy from argmax(Q_sa)
	for s in range(num_spaces):
		R_sa = [sum([prob * r for prob, s_, r, done in env.P[s][a]]) for a in range(num_actions)]
		Q_sa = [R_sa[a] + gamma * sum([prob * V[s_] for prob, s_, r, done in env.P[s][a]]) for a in range(num_actions)]
